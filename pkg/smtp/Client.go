...
func (c *client) init(conn net.Conn, clientID uint64, ep *EnvelopePool) {
	c.conn = conn
...
func (c *client) parsePath(in []byte, p pathParser) (Address, error) {
	address := Address{}
	var err error
	if len(in) > rfc5321.LimitPath {
		return address, errors.New(Canned.FailPathTooLong.String())
	}
	if err = p(in); err != nil {
		return address, errors.New(Canned.FailInvalidAddress.String())
	} else if c.parser.NullPath {
		// bounce has empty from address
		address = Address{}
	} else if len(c.parser.LocalPart) > rfc5321.LimitLocalPart {
		err = errors.New(Canned.FailLocalPartTooLong.String())
	} else if len(c.parser.Domain) > rfc5321.LimitDomain {
		err = errors.New(Canned.FailDomainTooLong.String())
	} else {
		address = Address{
